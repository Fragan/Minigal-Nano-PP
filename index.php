        ...
        if ($file != "." && $file != ".." && $file != "folder.jpg") {
            ...
            if (preg_match("/.jpg$|.gif$|.png$/i", $file)) {
                ...
                if (file_exists($currentdir . "/metadata.txt")) {
                    ...
                    // Example: IMG_0259.jpg|My cat::My cat like to <i>roll</i> on the floor.
                    // If file is not provided, image filename will be used instead.
                    $file_handle = fopen($currentdir . "/metadata.txt", "rb");
                    while (!feof($file_handle)) {
                        $line_of_text = fgets($file_handle);
                        $parts = explode('/n', $line_of_text);
...
if (sizeof($dirs) > 0) {
    ...
    if (strtoupper($sortdir_folders) == "DESC")
        array_multisort($$sorting_folders, SORT_DESC, $name, SORT_DESC, $dirs);
    else
        array_multisort($$sorting_folders, SORT_ASC, $name, SORT_ASC, $dirs);
}
if (sizeof($files) > 0) {
    ...
    if (strtoupper($sortdir_files) == "DESC")
        @array_multisort($$sorting_files, SORT_DESC, $name, SORT_ASC, $files);
    else
...
if ($comment == "") {
    ...
    if (file_exists($currentdir . "/metadata.txt")) {
        $file_handle = fopen($currentdir . "/metadata.txt", "rb");
        while (!feof($file_handle) && $aa == "true") {
            $line_of_text = fgets($file_handle);
            $parts = explode('/n', $line_of_text);
            foreach ($parts as $img_capts) {
                list($img_filename, $img_caption) = explode('|', $img_capts);
    ...
    if ($directory == "") {
        $director = strpbrk($currentdir, '/');
        if ($director != "") {
            $directo = explode("/", $director);
            $directory = $directo[1];
        }
    }
}
